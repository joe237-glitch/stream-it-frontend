import { useState } from 'react'
import { useSignIn, useSignUp } from '@clerk/clerk-react'
import { useAuth } from '../context/AuthContext'
import { Auth } from '../api/client'

export default function GoogleSignInButton({ onError }) {
  const { signIn, isLoaded: signInLoaded } = useSignIn()
  const { signUp, isLoaded: signUpLoaded } = useSignUp()
  const { login } = useAuth()
  const [loading, setLoading] = useState(false)

  const handleGoogle = async () => {
    if (!signInLoaded || !signUpLoaded || loading) return
    setLoading(true)

    try {
      // Start OAuth flow with Clerk
      await signIn.authenticateWithRedirect({
        strategy: 'oauth_google',
        redirectUrl: '/sso-callback',
        redirectUrlComplete: '/oauth-complete',
      })
    } catch (err) {
      // If user doesn't exist in Clerk, try sign up
      if (err?.errors?.[0]?.code === 'form_identifier_not_found') {
        try {
          await signUp.authenticateWithRedirect({
            strategy: 'oauth_google',
            redirectUrl: '/sso-callback',
            redirectUrlComplete: '/oauth-complete',
          })
        } catch (e) {
          onError?.(e.errors?.[0]?.message || 'Erreur Google')
          setLoading(false)
        }
      } else {
        onError?.(err.errors?.[0]?.message || 'Erreur Google')
        setLoading(false)
      }
    }
  }

  return (
    <button
      type="button"
      onClick={handleGoogle}
      disabled={loading || !signInLoaded}
      className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white font-medium transition-all disabled:opacity-50"
    >
      {loading ? (
        <span className="text-sm text-slate-400">Connexion en cours...</span>
      ) : (
        <>
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A11.96 11.96 0 0 0 1 12c0 1.94.46 3.77 1.18 5.43l3.66-2.84z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          <span className="text-sm">Continuer avec Google</span>
        </>
      )}
    </button>
  )
}
